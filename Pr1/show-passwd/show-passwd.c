...
/**
 * Pregunta 1) En C no se pueden hacer asignaciones con =. Usamos clone_string para asegurarnos de que siempre
 * hay espacio suficiente para almacenar dicho string, cosa que puede no ocurrir con strcpy()
 * 
 * Pregunta 2) line se va actualizando a posiciones detrás de cada token
 * 
 **/
...
static int show_passwd(struct options* options)
{
	...
	for (i=0; i<nr_entries; i++) {
		...
		case VERBOSE_MODE:
			if (options->show_mode == NORMAL_MODE){
...
int main(int argc, char *argv[])
{
	...
	options.show_mode = NORMAL_MODE;

	/* Parse command-line options */
	while((opt = getopt(argc, argv, "hvpci:o:")) != -1) {
		switch(opt) {
		case 'h':
			fprintf(stderr,"Usage: %s [ -h | -v | -p | -c | -i <inputfile> | -o <output_file> ]\n",argv[0]);
			exit(0);
		case 'v':
			options.output_mode=VERBOSE_MODE;
			break;
	...
	options.infile = fopen("/etc/passwd", "r");
		if (options.infile == NULL){
			fprintf(stderr, "The input file %s could not be opened: ",
				        optarg);
				perror(NULL);
